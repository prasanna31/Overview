...
with st.expander("Machine Learning?"):
    ...
    with st.expander("4. Supervised Learning"):
        ...
        st.set_page_config(page_title="Machine Learning Concepts", layout="wide")

        with st.expander("4. Machine Learning Concepts"):

            with st.expander("4.1 Regression"):
                st.markdown("""
        **Regression** is a supervised learning task where the goal is to predict a **continuous numeric output** from input features.

        **Types:**
        1. **Linear Regression:** Predict output as a linear combination of inputs.
        """)
        ...
        - **Loss function (Mean Squared Error):""")
                ...
                st.markdown("""
        **Intuition:** Fit a line (or hyperplane in higher dimensions) that minimizes squared vertical errors.  
        **Geometric view:** The plane passes through the data points such that the sum of squared vertical distances is minimized.
        ...
        """)
            ...
            with st.expander("4.2 Classification"):
                st.markdown("""
        **Classification** is a supervised learning task where the goal is to predict **discrete labels**.
        ...
        **Algorithms:**
        ...
        """)
                ...
                st.markdown("""
        - Decision Trees, Random Forests, Support Vector Machines, k-NN

        **Loss function (Cross-Entropy / Log Loss):""")
                st.latex(r"\text{Loss} = -\frac{1}{n} \sum_{i=1}^{n} \left[y_i \log(\hat{y}_i) + (1-y_i) \log(1-\hat{y}_i)\right]")

        st.markdown("""
        **Intuition:** Find boundaries that separate classes correctly.  
        **Geometric view:** Hyperplanes or curves divide clusters of points in feature space.
        """)

        with st.expander("4.3 Bias-Variance Tradeoff"):
                st.markdown("""
        **Bias-Variance Tradeoff** explains sources of error in supervised learning.

        **Error decomposition:**
        """)
                st.latex(r"\text{Expected Error} = \text{Bias}^2 + \text{Variance} + \text{Irreducible Error}")
        ...
        **Intuition:** Balance model complexity to minimize total error.  
        **Geometric view:**  
        - Underfitting → model too flat, misses patterns.  
        - Overfitting → model wiggly, fits noise in the data.
        """)
        ...
        **Intuition:** Choose metrics based on problem; e.g., for imbalanced classes, accuracy can be misleading.
        """)

        with st.expander("4.5 Regularization Techniques"):
                st.markdown("""
        **Regularization** adds a penalty to the loss function to prevent overfitting by discouraging overly complex models.
        """)

                st.markdown("**L1 Regularization (Lasso):**")
                st.latex(r"\text{Loss}_{L1} = \text{MSE} + \lambda \sum_{j=1}^{n} |\beta_j|")
                st.markdown("→ Can set some coefficients exactly to 0, performing feature selection.")
                st.markdown("**Intuition:** L1 reduces variance and prevents overfitting by selecting only important features.")
                st.markdown("**Geometric view:** Coefficients are projected onto axes, producing a sparse solution.")

                st.markdown("**L2 Regularization (Ridge):**")
                st.latex(r"\text{Loss}_{L2} = \text{MSE} + \lambda \sum_{j=1}^{n} \beta_j^2")
                st.markdown("→ Shrinks coefficients towards zero but rarely exactly zero.")
                st.markdown("**Intuition:** Keeps all coefficients small to reduce overfitting while retaining features.")
                st.markdown("**Geometric view:** Coefficients are smoothly pulled towards origin in feature space.")

                st.markdown("**Elastic Net (L1 + L2):**")
